...
const MAX_CLAIM_LIMIT = limits.DEFAULTS.MAX_CLASSROOM_CLAIM_LIMIT;
...
function claimQuotaPatch(body) {
  ...
  const classLimit = parseOptionalCount(body.claimLimit, {
    min: MIN_CLAIM_LIMIT, max: MAX_CLAIM_LIMIT,
  });
  if (classLimit === 'invalid') {
    return {
      error: `Classroom capacity must be a whole number between ${MIN_CLAIM_LIMIT} and ` +
             `${MAX_CLAIM_LIMIT.toLocaleString()} ClaimChecks. Leave it blank to size it ` +
...
router.patch('/rooms/:id', requireConfigured, requireEducator, async (req, res) => {
  try {
    ...
    if (Object.keys(quota.patch).length) {
      const claimsUsed = Number(room.claims_used) || 0;
      ...
      if (nextLimit < claimsUsed) {
        return res.status(400).json({
          error: `That works out to ${nextLimit} ClaimChecks, but this classroom has already used ${claimsUsed}.`,
        });
      }
    }
