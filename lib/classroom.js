...
function ownerView(classroom) {
  return {
    ...
    // Claim quotas. Null means "follow the server default", which is resolved
    // per request rather than frozen into the row, so the dashboard reports the
    // stored value and the effective one separately.
    claimsUsed: Number(classroom.claims_used) || 0,
    ...
    effectiveClaimLimit: effectiveClaimLimit(classroom),
...
function effectiveClaimLimit(classroom) {
  ...
  if (classroom.claim_limit !== null && classroom.claim_limit !== undefined && Number.isFinite(explicit)) {
    return explicit;
  }
...
function effectiveClaimLimitPerStudent(classroom) {
  ...
  if (classroom.claim_limit_per_student !== null && classroom.claim_limit_per_student !== undefined && Number.isFinite(explicit)) {
    return explicit;
  }
...
const COLUMNS = 'id,owner_id,display_name,access_code,session_secret,created_at,expires_at,active,token_budget,tokens_used,analyses_run,searches_used,claims_used,claim_limit,claim_limit_per_student,expected_students';
 ...
 */
async function createClassroom({ ownerId, displayName, expiresAt, tokenBudget, claimQuota = {} }) {
