...
const REPORT_LABELS = {
  en: {
    ...
    relevanceWarning:
      'No source below tests this claim directly. Every source answers a related but ' +
      'different question, so this verdict is provisional.',
    verdicts: { supported: 'Supported', contradicted: 'Contradicted', unclear: 'Unclear' },
    confidences: { high: 'High', medium: 'Medium', low: 'Low' },
    noTargeting: 'This claim is not identity-related.',
    aboutNotTargeting: 'This claim is about identity, but it does not itself target an identity group.',
    readoutAbout: 'About identity: Yes. Targets a group: No.',
    readoutTargeting: 'About identity: Yes. Targets a group: Yes.',
  ...
  es: {
    ...
    identityHeading: 'Lente de identidad',
    ...
    relevanceWarning:
      'Ninguna fuente a continuación pone a prueba esta afirmación directamente. Todas ' +
      'responden a una pregunta relacionada pero distinta, así que este veredicto es provisional.',
    verdicts: { supported: 'Respaldada', contradicted: 'Cuestionada', unclear: 'No está claro' },
    confidences: { high: 'Alta', medium: 'Media', low: 'Baja' },
    noTargeting: 'Esta afirmación no se relaciona con la identidad.',
    aboutNotTargeting: 'Esta afirmación trata sobre la identidad, pero no ataca a ningún grupo de identidad.',
    readoutAbout: 'Trata sobre la identidad: Sí. Ataca a un grupo: No.',
    readoutTargeting: 'Trata sobre la identidad: Sí. Ataca a un grupo: Sí.',
...
/** Label for what KIND of source this is. Empty for results predating source_type. */
function sourceTypeLabel(type, L) {
  const t = str(type).toLowerCase();
  return (t && t !== 'other' && L.sourceTypes[t]) || '';
}

/**
 * Note flagging a source that answers a nearby question instead of the claim.
 *
 * Empty for "direct" sources and for results predating the relevance field, so
 * the common case stays uncluttered and the exception stands out.
 */
function relevanceNote(item, L) {
  const r = str(item.relevance).toLowerCase();
  if (r !== 'related' && r !== 'background') return '';
  const addresses = str(item.addresses);
  return addresses ? `${L.relevance[r]} ${addresses}` : L.relevance[r];
}
...
function mapEvidence(items, L) {
  ...
  return items
    .map(it => ({
      ...
      sourceType: sourceTypeLabel(it.source_type, L),
      relevanceNote: relevanceNote(it, L),
...
function buildIdentityConcerns(lens, L) {
  ...
  const patternItems = Array.isArray(lens.patterns_observed) ? lens.patterns_observed : [];
  // Results saved before the lens split carry one `targets_identity` boolean
  // that conflated "about identity" with "targets a group" — read it as the latter.
  const legacy = Boolean(lens.targets_identity);
  const targeting = (typeof lens.contains_targeting === 'boolean' ? lens.contains_targeting : legacy)
    || patternItems.length > 0;
  const about = typeof lens.about_identity === 'boolean' ? lens.about_identity : legacy;
  if (!targeting) {
    const base = str(lens.analysis) || (about ? L.aboutNotTargeting : L.noTargeting);
    return about ? `${L.readoutAbout} ${base}` : base;
  }
  const parts = [L.readoutTargeting];
...
function normalizeResult(raw = {}, meta = {}) {
  ...
  return {
    ...
    // Set when the analysis had to qualify its verdict because nothing found
    // tests the claim directly. Carried into the report so a printed copy says
    // so too, not just the screen.
    relevanceWarning: (raw._meta && raw._meta.relevance && raw._meta.relevance.verdict_rests_on_indirect)
      ? L.relevanceWarning : '',
...
async function buildPdf(report) {
  ...
  // Evidence
  if (report.relevanceWarning) text(report.relevanceWarning, { size: 9.5, f: bold, color: MUTED });
  ...
  function drawEvidence(items) {
    for (const e of items) {
      ...
      if (e.relevanceNote) text(e.relevanceNote, { size: 9, f: bold, color: MUTED, gap: 1 });
      const sourceParts = [];
      if (e.sourceName) sourceParts.push(e.sourceName);
      if (e.sourceType) sourceParts.push(e.sourceType);
...
async function buildDocx(report) {
  ...
  const evidenceBlock = (label, items) => {
    ...
    for (const e of items) {
      ...
      if (e.relevanceNote) children.push(new Paragraph({
        children: [new TextRun({ text: e.relevanceNote, size: 18, bold: true, color: '92400E' })],
        spacing: { after: 30 },
      }));
      const meta = [];
      if (e.sourceName) meta.push(e.sourceName);
      if (e.sourceType) meta.push(e.sourceType);
  ...
  if (report.relevanceWarning) children.push(p(report.relevanceWarning, { bold: true, color: '92400E' }));
