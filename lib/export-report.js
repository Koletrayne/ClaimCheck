...
function confidenceLabel(c) {
  ...
  return { high: 'High', medium: 'Medium', low: 'Low' }[v] || '';
}
...
async function buildPdf(report) {
  ...
  if (report.confidence) text('Confidence: ' + report.confidence, { size: 9.5, color: MUTED });
  ...
  if (report.contextWarning || report.identityConcerns) {
    ...
    if (report.contextWarning) text('Warning: ' + report.contextWarning, { color: rgb(0.6, 0.3, 0.05) });
...
async function buildDocx(report) {
  ...
  if (report.confidence) children.push(p('Confidence: ' + report.confidence, { italics: true, color: '666666' }));
  ...
  if (report.contextWarning || report.identityConcerns) {
    ...
    if (report.contextWarning) children.push(p('Warning: ' + report.contextWarning, { color: '9A4D00' }));
