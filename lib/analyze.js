...
async function analyzeClaim({ text, sourceUrl, academicMode = false, includeSecondaryClaims = false, snapshot = false, includeContextLens = true, language = DEFAULT_LANGUAGE }) {
  ...
  if (!parsed) {
    ...
    //
    // Both carry the usage the attempt spent. These are the MOST expensive way
    // an analysis can fail — the provider ran to completion and was paid in
    // full, and only the last step went wrong — so a caller that could not see
    // the cost here would let a classroom fail this way indefinitely with its
    // token ceiling untouched. runAgenticLoop attaches usage to transport
    // failures for the same reason; this covers the ones it never sees.
    const err = new Error(result.truncated
      ? (snapshot
      ...
      : 'Model did not return valid JSON. Try again with more text.');
 ...
 * One line per API call, so the cost of an analysis can be attributed to the
 * call that incurred it instead of inferred from a total.
 *
 * This exists because the classroom allowance was once sized at ~3,300 tokens
 * per analysis when the real figure is closer to 29,000, and nothing in the
 * logs would have shown that. The surprise, once measured, is that the whole
 * analysis is ONE API call: the web_search tool resolves server-side, and every
 * page it reads is billed as input on the single request that asked for it. So
 * the number worth watching is input tokens per search, and that is what this
 * prints.
 *
 * Off in production unless CLAIMCHECK_TOKEN_DIAGNOSTICS is set. Logs counts and
 * a model name — never the claim, the messages, or the search results.
 */
function tokenDiagnosticsEnabled() {
...
function logCallUsage(callNumber, response) {
  if (!tokenDiagnosticsEnabled()) return;
  const u = (response && response.usage) || {};
  const searches = (u.server_tool_use && u.server_tool_use.web_search_requests) || 0;
  console.log(
    `[analyze:diag] call ${callNumber} model=${MODEL} stop=${response && response.stop_reason} ` +
    `input=${u.input_tokens || 0} output=${u.output_tokens || 0} ` +
    `cache_read=${u.cache_read_input_tokens || 0} cache_create=${u.cache_creation_input_tokens || 0} ` +
    `searches=${searches} charged=${totalTokens({
