...
- Include at most 2 items in "supporting_evidence" and at most 2 in "contradicting_evidence" — only the single strongest source on each side.
- Still set "claim_text", "verdict", "confidence", and assess identity targeting: set identity_lens.targets_identity and a ONE-sentence identity_lens.analysis (leave its arrays empty unless targeting is clearly present). Set contextLens.contextWarning only if the claim involves identity-based hate, stereotypes, or harmful framing; otherwise "".
...
OUTPUT LANGUAGE — SPANISH (español): The user selected Spanish. Write ALL human-readable output in natural, neutral Latin American Spanish appropriate for high school students and teachers in the United States (California). This applies to: claim_text, original_excerpt, the breakdown fields what/who/when/where/evidence_required and breakdown.evidence_found.note, every evidence "summary" and "credibility_note", verdict_explanation, bottom_line, uncertainty_notes, every reflection_questions item, secondary_claims, all identity_lens text (analysis, caution_note, and each pattern's explanation), and every contextLens field (backgroundSnapshot, keyContext, whyContextMatters, missingInformation, reflectionQuestions, contextWarning).
...
- Keep proper nouns, organization names, publication names, and each "source_name" in their original form. Do NOT translate URLs. Do NOT invent translated titles for sources. If you would quote a source, preserve the quotation in its original language; when you summarize instead, write the summary in Spanish.
...
async function runAgenticLoop({ system, userMessage, tools, maxTurns, maxTokens = 6144 }) {
  ...
  for (let turn = 0; turn < maxTurns; turn++) {
    const response = await callAnthropic({ system, messages, tools, maxTokens });
  ...
  return { finalText, searchesUsed, usage };
}
...
module.exports = { analyzeClaim, normalizeLanguage, SUPPORTED_LANGUAGES, totalTokens, emptyUsage };
